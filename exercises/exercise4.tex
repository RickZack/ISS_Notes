...
\com{
    ...
    A -> RADIUS doesn't provide authentication by itself, instead it relies on PAP CHAP or EAP \\ 
    B -> Authentication protocol of PPP \\
...
\end{figure}
\com{   
    A -> RADIUS itself doesn't authenticate the connection and we need authentication to the NAS, so we don't use RADIUS anyway. go to Q8 
}
...
\end{itemize}
\com{EAP support a lots of stuff}
...
\end{itemize}
\com{This question should help you understand previous questions about NAS and RADIUS \\
    A -> no, is B \\
    C -> RADIUS relies over this PPP (protocol) \\ 
    E -> see the email example in the notes
}
...
\end{figure}
\com{Here is where RADIUS is needed}
...
\end{itemize}
\com{ $password \oplus md5(key+authenticator)$ }
