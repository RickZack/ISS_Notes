...
\begin{enumerate}
    \textbf{\item[A.] symmetric encryption of data by splitting data in blocks}
...
\end{enumerate}
\textcolor{Comment}{A -> $C_{i} = enc(K,P_{i})$ \\ B -> $P_{i} = enc^{-1}(K,C_{i})$, \\ E,H -> identical block encrypt equally, 
...
\end{enumerate}
\textcolor{Comment}{A,B do not damage Alice Bob communication}
...
\end{enumerate}
\textcolor{Comment}{A -> no, B and C no, we don use it for encrypt (since the key for decrypt is public)}
