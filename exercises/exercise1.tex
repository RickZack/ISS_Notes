...
\item[B.] integrity
\item[C.] availability
\textbf{\item[D.] peer authentication}
...
\begin{enumerate}
    ...
    \textbf{\item[C.] Bob must verify that the data really comes from Alice (for
...
\begin{enumerate}
    \textbf{\item[A.] Alice authenticates to every system run by Bob and each of the two systems of Bob authenticate to Alice}
...
\textcolor{Comment}{The mutual authentication requirement is not met by B, C and D. We don't care about Bob so exclude E}
...
\begin{enumerate}
    \textbf{\item[A.] an attack in which some data (in clear or protected for confidentiality) can be intercepted and then sent more than once to the destination}
...
\textcolor{Comment}{We can exclude option C. Although it may be annoying it's not an attack.\\
...
\begin{enumerate}
    \textbf{\item[A.] an attack in which data (in clear or protected) is intercepted while in transmission}
...
\end{enumerate}
\textcolor{Comment}{Sniffing is a passive attack, the C option describes a MITM attack. B no because we 'could' decrypt the text}
...
\begin{enumerate}
    ...
    \textbf{\item[B.] an attack in which an attacker modifies the IP source address and sends it to a destination}
...
\end{enumerate}
\textcolor{Comment}{A -> MITM/data spoofing; B -> IP spoofing is needed to hide our identity. }
...
\begin{enumerate}
    ...
    \textbf{\item[C.] avoid the use of the IP address as "credential" to authenticate and get access to any remote resources}
    \item[D.] install (and regularly update) an anti-malware application on each device
\end{enumerate}
\textcolor{Comment}{A -> It's not about the destination; \\B -> we can have IP spoofing in a client-server communication(with different subnets);
\\D -> wtf}


\textbf{Q12 - What is a Denial of Service attack?}
\begin{enumerate}
    \textbf{\item[A.] an attack in which the attacker keeps a host busy by exhausting its resources (e.g. mail) so that it cannot provide its services}
    \textbf{\item[B.] an attack in which the attacker keeps a host busy by flooding it with traffic (e.g. DNS or ICMP) so that it cannot provide its services}
    \textbf{\item[C.] an attack in which an attacker keeps a host busy by exhausting its resources (e.g. by injecting malware that makes continuous calculations) so that to block the use of the host}
    \item[D.] an attack in which an attacker denies the use of a host to a user because he/she is not authorized
\end{enumerate}
\textcolor{Comment}{Denial-of-service refers to keep a host busy so that it cannot provide its services.\\ A single user (D) without service does not define a DOS attack.}


\textbf{Q13 - What is Distributed Denial of Service (DoS) attack?}
\begin{enumerate}
    \textbf{\item[A.] attack in which the attacker (master) exploits multiple daemons installed on compromised nodes to run (upon command) a software implementing the DoS attack against one victim}
...
\begin{enumerate}
    ...
    \textbf{\item[B.] Denial of Service}
    \textbf{\item[C.] redirect Alice to fake web sites}
...
%B is correct but-> I assume that a shadow server could lead to a denial-of-service attack on a service provider. 
%However, I do not understand how it could cause a denial-of-service attack on Alice. 
...
\begin{enumerate}
    ...
    \textbf{\item[C.] server authentication, data authentication, confidentiality of the data exchanged, integrity of the data exchanged, and serialization of each packet}
...
\end{enumerate}
\textcolor{Comment}{A - miss serialization\\ B - miss data authentication\\ D - privacy has a different meaning from confidentiality, has a more legal nounce(I don't know how to explain it clearly)}
...
\begin{enumerate}
    ...
    \textbf{\item[D.] the ICT resources, data, people, and location used in providing the services offered by the company}
\end{enumerate}
\textcolor{Comment}{For each services -> ICT resources, data, people and location}
...
\begin{enumerate}
    ...
    \textbf{\item[D.] weaknesses in the design, implementation, configuration, and management that could harm assets if exploited by an attacker or by occurrence of unintentional events (natural disasters, mistakes performed by individuals)}
\end{enumerate}
\textcolor{Comment}{no action but weaknesses, not only of software but of every aspect, vulnerable to unintentional events as well }

\textbf{Q18 - In risk analysis, a security control is:}
\begin{enumerate}
    \textbf{\item[A.] a set of operational and management processes, and security mechanisms (including software techniques, algorithms, and protocols) used to protect against threats}
    \item[B.] functional and non-functional requirements that need to be satisfied in order to achieve security
    \textbf{\item[C.] a set of operational and management processes, and security mechanisms (including software techniques, algorithms, and protocols) adopted to reduce risks}
\end{enumerate}
\textcolor{Comment}{Figure of chapter 1.8; B is security requirements}

\textbf{Q19 - In risk analysis, a risk is:}
\begin{enumerate}
    \textbf{\item[A.] a qualitative (e.g., low, medium, high, very high) or quantitative (e.g. 10, 15, 25) value calculated based on the impact and the probability of occurrence of a security event}
...
\begin{enumerate}
    \textbf{\item[A.] documents expressing ‘how’ you implement the policies, both for the technical details (such as specific techniques, algorithms used) and organizational details}
...
\begin{enumerate}
    \textbf{\item[A.] delegate responsibility}
