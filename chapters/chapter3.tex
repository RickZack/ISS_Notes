...
64-bit passwords are a compromise, but you have yet to enter them. Entering 64-bit as hexadecimal characters means 16 hexadecimal characters. So, normally they are entered as a sequence of six short English words chosen from a dictionary of 2048 (e.g., 0 = A, 1 = ABE, 2 = ACE, 3 = ACT, 4 = AD, 5 = ADA). It means selecting 11 bits from the computed hash and using a dictionary which has some simple words corresponding to the combinations (2048 in this case), \textbf{and the client and server must share the same dictionary}. For example:
\begin{itemize}
    \item Password (text): "YOU SING A NICE OLD SONG," but not because it is a password but because “YOU” is one of the words in the dictionary and it represents 11 bits. In total, there are 6 words, which means 66-bits (more than 64, so it is good).
    \item Password (numeric): 1D6E5001884BD711 (hex) or 2,120,720,442,049,943,313 (decimal).
\end{itemize}
This is just an example of how to encode a long bit string in a user-friendly way. See Figure 39 for Password generation and verification using the S/KEY system.
